...
func (r *PaymentRepository) UpdateStatusIfPending(ctx context.Context, id uuid.UUID, newStatus model.PaymentStatus) (bool, error) {
	result, err := r.db.Exec(ctx,
		`UPDATE payments SET status = $1, updated_at = NOW() WHERE id = $2 AND status = $3`,
		newStatus, id, model.PaymentStatusPending)
	if err != nil {
